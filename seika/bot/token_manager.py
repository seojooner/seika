...
class TokenManager(Item):
    ...
    async def _create_bot(self) -> Union[str, None]:
        ...
        async with fsm.Conversation(self._app, "@BotFather", True) as conv:
            ...
            await conv.ask(f"Seika UserBot of {utils.get_display_name(self._all_modules.me)[:45]}")
            await conv.get_response()

            bot_username = f"seika_{utils.random_id(6)}_bot"
            ...
            await conv.get_response()
# надо аву поставить до папки seika, нев ней а до нее
    ...
    async def _revoke_token(self) -> str:
        ...
        async with fsm.Conversation(self._app, "@BotFather", True) as conv:
            ...
            for row in response.reply_markup.keyboard:
                for button in row:
                    search = re.search(r"@seika_[0-9a-zA-Z]{6}_bot", button)
                    if search:
